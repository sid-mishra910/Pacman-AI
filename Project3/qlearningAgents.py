...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        # print(len(self.qvalues.sortedKeys()))
        keys = self.qvalues.keys()
        # print(keys)
        if len(keys) > 0:
...
class PacmanQAgent(QLearningAgent):
    ...
    def __init__(self, epsilon=0.1,gamma=0.7,alpha=0.3, numTraining=0, **args):
        """
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        w = self.getWeights()
        for key in keys:
            # print('This is in Qvalue', i)
            q_value = q_value + (features[key] * w[key])
        return q_value
        util.raiseNotDefined()
    ...
    def update(self, state, action, nextState, reward):
        ...
        w = self.getWeights()
        for key in keys:
          # print('This is in upade',key)
          w[key] += (self.alpha * diff * features[key])
