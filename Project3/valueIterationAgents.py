...
class ValueIterationAgent(ValueEstimationAgent):
          ...
          for state in all_states:
            if self.mdp.isTerminal(state):
              ...
              for actions in possible_actions:
                q_value = self.computeQValueFromValues(state, actions)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next_state, prob in state_prob_pair:
          reward_state = self.mdp.getReward(state, action, next_state)
          fin_sum = fin_sum + (reward_state * prob) + (prob * self.discount *  self.values[next_state])
        return fin_sum
